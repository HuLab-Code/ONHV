...
for(i in 1:6){
  ...
  distanr<-c()
  for(k in 1:datacol){
    aa<-c(Edge[[i]][k][[1]][1]/50,Edge[[i]][k][[1]][2]/36)
    b1<-c(a[[2*i-1]][1]/50,a[[2*i-1]][2]/36)
    b2<-c(a[[2*i]][1]/50,a[[2*i]][2]/36)
    distanl[k]<-Dis(aa,b1)
    distanr[k]<-Dis(aa,b2)
  }
  sumlength<-sumlength+min(distanl)
  sumlength<-sumlength+min(distanr)
}
sumlength<-sumlength/12
sumlength